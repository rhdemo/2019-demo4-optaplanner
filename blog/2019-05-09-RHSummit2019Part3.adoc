...
:awestruct-tags: [vehicle routing, maintenance scheduling, integration, production, benchmark, simulation, load testing]
...
there is a first broken machine.
...
The purpose of load testing is to be able to tell if the system under test is able to handle certain load. If there are
...
The devil was hidden in Infinispan and also in the way we contacted Infinispan for machines healths every 40 milliseconds.
...
about machines healths to be retrieved from Infinispan.
...
They helped us understand what we could expect and build a good level of confidence about what we wanted to show.

Let me thank Guilherme Baufaker Rego for his great job on load testing the platform.

link::2019-05-09-RHSummit2019Part1.html[Part 1 by Geoffrey]
link::2019-05-09-RHSummit2019Part2.html[Part 2 by Musa]
