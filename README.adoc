...
First make sure docker daemon is running, if not type `sudo systemctl start docker` or add another environment variable
`-Ddocker.host=DOCKER_DAEMON_URL`.

    mvn clean package -Prelease fabric8:build fabric8:push \
        -Ddocker.username=... -Ddocker.password=...

Where username and passowrd are those of your quay.io account.
